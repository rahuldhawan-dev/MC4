...
{
    public partial class InitiativeSteps : DataElementPageWithRoles
    {
        #region Properties
        ...
        protected override void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            if (!String.IsNullOrEmpty(lbGoals.SelectedValue))
            {
                SqlDataSource1.SelectParameters["GoalIDs"].DefaultValue = string.Empty;

                foreach (ListItem item in lbGoals.Items)
                {
                    if (item.Selected)
                        SqlDataSource1.SelectParameters["GoalIDs"].DefaultValue += item.Value + ",";
                }
                SqlDataSource1.SelectParameters["GoalIDs"].DefaultValue =
                    SqlDataSource1.SelectParameters["GoalIDs"].DefaultValue.TrimEnd(',');
                sb.Append(" AND Goals > 0");
            }
