var ServiceInstallation = (function ($) {
  ...
  var si = {
    init: function () {
      URLS = { sapDeviceDetail: '../../SAP/SAPDeviceDetail/Index.json' };
      ELEMENTS = {
        ajaxError: $('#ajaxError'),
    ...
    search_callBack: function (d) {
      ...
      // TODO: Alex said he's gonna come fix this so it's a success bool. -Ross 6/13/2017
      if (SAPErrorCode.toUpperCase().indexOf('DEVICE STATUS IS INCOMPATIBLE') > -1 || SAPErrorCode.toUpperCase().indexOf('ENTER VALID SERIAL NO') > -1
        || SAPErrorCode.toUpperCase().indexOf('UNABLE TO LOCATE A VALID WORK ORDER') > -1 || SAPErrorCode.toUpperCase().indexOf('MUST PROVIDE A WORKORDER') > -1) {
        ELEMENTS.save.prop('disabled', false);
        ELEMENTS.fullForm.toggle(true);
        ELEMENTS.manufacturer.prop('readOnly', false);
        ELEMENTS.meterSize.prop('readOnly', false);
        ELEMENTS.serviceType.prop('readOnly', false);
        ELEMENTS.meterManufacturerSerialNumber.prop('readOnly', false);
        ELEMENTS.register1Dials.prop('readOnly', false);
        ELEMENTS.register1UnitOfMeasure.prop('readOnly', false);
        ELEMENTS.register1Size.prop('readOnly', false);
        ELEMENTS.register2Dials.prop('readOnly', false);
        ELEMENTS.register2UnitOfMeasure.prop('readOnly', false);
        ELEMENTS.register2Size.prop('readOnly', false);
        ELEMENTS.ajaxError.css('color', 'red');

        ELEMENTS.meterManufacturerSerialNumber.val('');
        ELEMENTS.manufacturer.val('');
        ELEMENTS.serviceType.val('');
        ELEMENTS.meterSize.val('');
        ELEMENTS.meterDeviceCategory.val('');
        ELEMENTS.materialNumber.val('');

        // Register 1 Details
        ELEMENTS.register1RFMIU.val('');
        ELEMENTS.register1EncoderId.val('');
        ELEMENTS.register1Size.val('');
        ELEMENTS.register1DeviceCategory.val('');
        ELEMENTS.register1Dials.val('');
        ELEMENTS.register1UnitOfMeasure.val('');
        ELEMENTS.register1CurrentRead.val('');
        ELEMENTS.register1ReadType.val('');

        // Register 2 Details
        ELEMENTS.register2RFMIU.val('');
        ELEMENTS.register2EncoderId.val('');
        ELEMENTS.register2DeviceCategory.val('');
        ELEMENTS.register2Dials.val('');
        ELEMENTS.register2UnitOfMeasure.val('');
        ELEMENTS.register2CurrentRead.val('');
        ELEMENTS.register1ReadType.val('');
        return;
      }
      ELEMENTS.ajaxError.css('color', '');
      ELEMENTS.save.prop('disabled', false);
      ELEMENTS.fullForm.toggle(true);
