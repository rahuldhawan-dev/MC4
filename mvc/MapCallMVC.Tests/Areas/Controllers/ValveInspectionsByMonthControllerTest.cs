...
{
    [TestClass]
    public class ValveInspectionsByMonthControllerTest : MapCallMvcControllerTestBase<ValveInspectionsByMonthController, ValveInspection, ValveInspectionRepository>
    {
        #region Init/Cleanup
        ...
        protected override void SetAutomatedTestOptions(AutomatedTestOptions options)
        {
            ...
            options.InitializeSearchTester = (tester) => {
                tester.TestPropertyValues[nameof(SearchValveInspectionsByMonth.Year)] = 2020;
            };
        }
        ...
        public override void TestControllerAuthorization()
        {
            ...
            {
                a.RequiresLoggedInUserOnly("~/Reports/ValveInspectionsByMonth/Search");
                a.RequiresLoggedInUserOnly("~/Reports/ValveInspectionsByMonth/Index");
            });
        }
        ...
        public override void TestIndexReturnsResults()
        {
            ...
            var search = new SearchValveInspectionsByMonth { Year = year, OperatingCenter = new[] { operatingCenter.Id } };

            var result = _target.Index(search) as ViewResult;
            var resultModel = ((IEnumerable<ValveInspectionsByMonthReport>)result.Model).ToList();

            MvcAssert.IsViewNamed(result, "Index");
            Assert.AreEqual(3, resultModel.Count());
            Assert.AreEqual(Valve.Display.SIZE_RANGE_SMALL_VALVE, resultModel[0].SizeRange);
            Assert.AreEqual(Valve.Display.SIZE_RANGE_LARGE_VALVE, resultModel[1].SizeRange);
            Assert.AreEqual("Total", resultModel[2].SizeRange);
        }
        ...
        public void TestIndexXMLSExportsExcel()
        {
            ...
            using (var helper = new ExcelResultTester(_container, result, true))
            {
                ...
                helper.AreEqual("Total", "SizeRange", 2);
            }
        }
