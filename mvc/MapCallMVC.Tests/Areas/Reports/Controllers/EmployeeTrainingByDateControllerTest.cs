...
{
    [TestClass]
    public class EmployeeTrainingByDateControllerTest : MapCallMvcControllerTestBase<EmployeeTrainingByDateController, TrainingRecordAttendedEmployee>
    {
        ...
        {
            var role = EmployeeTrainingByDateController.ROLE;

            Authorization.Assert(a => {
                a.RequiresRole("~/Reports/EmployeeTrainingByDate/Search/", role);
                a.RequiresRole("~/Reports/EmployeeTrainingByDate/Index/", role);
            });
        }

        #endregion

        private TrainingRecordAttendedEmployee CreateRecord(Employee employee = null)
        {
            employee = employee ?? GetEntityFactory<Employee>().Create();
            var record = GetEntityFactory<TrainingRecord>().Create(new
            {
                HeldOn = DateTime.Now.AddWeeks(-1)
            });
            ...
            record = Session.Load<TrainingRecord>(record.Id);
            return employee.AttendedTrainingRecords.Last();
        }
        ...
        public override void TestIndexReturnsResults()
        {
            ...
            Session.Flush();
            var search = new SearchEmployeeTrainingByDate();
            _target.ControllerContext = new ControllerContext();

            var bareResult = _target.Index(search);
            var result = bareResult as ViewResult;
            var resultModel = ((SearchEmployeeTrainingByDate)result.Model).Results.ToList();
