...
{
    public class MapCallMvcControllerAuthorizationTester : ControllerAuthorizationTester<MvcApplication, User, MapCallMvcControllerAuthorizationAsserter>
    {
        ...
        public MapCallMvcControllerAuthorizationTester(MvcApplicationTester<MvcApplication> appTester, ISession session, ITestDataFactoryService testFactoryService, IContainer container)
        ...
        protected override MapCallMvcControllerAuthorizationAsserter CreateAsserter()
        {
            return new MapCallMvcControllerAuthorizationAsserter(_application, _factoryService, _session, _container);
        }

        #endregion
    }

    public class MapCallMvcControllerAuthorizationAsserter : ControllerAuthorizationAsserter<MvcApplication, User>
    {
        #region Private Members
        ...
        public MapCallMvcControllerAuthorizationAsserter(MvcApplicationTester<MvcApplication> appTester, ITestDataFactoryService testFactoryService, ISession session, IContainer container) : base(appTester, testFactoryService, container)
        {
            _session = session;
        }
        ...
        /// <summary>
        /// Asserts that User.IsUserAdmin must be true for a user to access a path.
        /// </summary>
        /// <param name="requestPath"></param>
        public void RequiresUserAdminUser(string requestPath)
        {
            // Tests that anon access is not allowed and that 
            // a user needs to be logged in. Includes the hacky
            // workaround to disable other global filters.
            RequiresUserToBeLoggedIn(requestPath, true);
            
            var route = Request.RouteContext;
            var actionAttr = route.ActionDescriptor.GetCustomAttributes(typeof(RequiresUserAdminAttribute), true);

            if (actionAttr.Any())
            {
                Reset(requestPath, UserType.Normal);
                User.IsUserAdmin = false;
                RealAssert.IsFalse(Request.IsAuthorized, "Request for '{0}' was authorized for non-user-admin user. It should not be authorized.", requestPath);

                Reset(requestPath, UserType.Normal);
                User.IsUserAdmin = true;
                RealAssert.IsTrue(Request.IsAuthorized, "Request for '{0}' was not authorized for user-admin user. It should be authorized.", requestPath);
            }
        }
