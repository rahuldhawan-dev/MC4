    ...
    public class AssetTypeControllerTest : MapCallMvcControllerTestBase<AssetTypeController, AssetType>
    {
        ...
        public override void TestControllerAuthorization()
        {
            Authorization.Assert(a => {
                ...
                a.RequiresLoggedInUserOnly("~/AssetType/ByStateOrOperatingCenterId/");
            });
        }
        ...
        [TestMethod]
        public void TestByStateOrOperatingCenterIdReturnsAssetTypes()
        {
            var state = GetFactory<StateFactory>().Create();
            var opcntr1 = GetFactory<OperatingCenterFactory>().Create(new { State = state });
            var opcntr2 = GetFactory<OperatingCenterFactory>().Create(new { State = state, OperatingCenterCode = "NJ4" });
            var validAssetType1 = GetFactory<FacilityAssetTypeFactory>().Create();
            var validAssetType2 = GetFactory<EquipmentAssetTypeFactory>().Create();
            var invalid = GetFactory<HydrantAssetTypeFactory>().Create();
            validAssetType1.OperatingCenterAssetTypes.Add(new OperatingCenterAssetType { OperatingCenter = opcntr1, AssetType = validAssetType1 });
            ...
            var result = (CascadingActionResult)_target.ByStateOrOperatingCenterId(null, opcntr1.Id);
            ...
            result = (CascadingActionResult)_target.ByStateOrOperatingCenterId(state.Id, null);
