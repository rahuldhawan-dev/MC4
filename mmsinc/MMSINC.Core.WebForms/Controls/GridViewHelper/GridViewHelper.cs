...
namespace MMSINC.Controls.GridViewHelper
{
    ...
    public class GridViewHelper
    {
        ...
        private const string USE_ADEQUATE_METHOD_TO_REGISTER_THE_SUMMARY =
        ...
        private const string SUPPRESS_GROUP_ALREADY_DEFINED =
        ...
        public GridViewHelper(GridView grd, bool useFooterForGeneralSummaries) : this(grd, useFooterForGeneralSummaries,
        ...
        public GridViewHelper(GridView grd, bool useFooterForGeneralSummaries, SortDirection groupSortDirection)
        {
            ...
            mGrid.RowDataBound += RowDataBoundHandler;
        }
        ...
        {
            return RegisterSummary(column, String.Empty, operation);
        }
        ...
        {
            return RegisterSummary(column, String.Empty, operation, groupName);
        }
        ...
        public GridViewSummary RegisterSummary(string column, CustomSummaryOperation operation,
        ...
        private GridViewRow InsertGridRow(GridViewRow beforeRow, GridViewGroup g)
        {
            ...
            TableCell cell;
            TableCell[] tcArray;
            int visibleColumns = GetVisibleColumnCount();
        ...
        private void RowDataBoundHandler(object sender, GridViewRowEventArgs e)
        {
            foreach (GridViewGroup g in mGroups)
            {
                ...
                else if (e.Row.RowType == DataControlRowType.Pager)
                {
                    ...
                    originalCell.ColumnSpan = GetVisibleColumnCount();
                }
            }
        ...
        private void GenerateGroupSummary(GridViewGroup g, GridViewRow row)
        {
            ...
            foreach (GridViewSummary s in g.Summaries)
            {
                if (s.Automatic)
                {
                    ...
                    newRow.Cells[colIndex].Text = GetFormatedString(s.FormatString,
                        GetColumnFormat(GetColumnIndex(s.Column)), s.Value);
                }
            }
        ...
        private void GenerateGeneralSummaries(GridViewRowEventArgs e)
        {
            ...
            if (!HasAutoSummary(mGeneralSummaries))
            {
            ...
            foreach (GridViewSummary s in mGeneralSummaries)
            {
                ...
                row.Cells[colIndex].Text =
                    GetFormatedString(s.FormatString, GetColumnFormat(GetColumnIndex(s.Column)), s.Value);
            }
        ...
        private bool ColumnHasSummary(int colindex, GridViewGroup g)
        {
            ...
            string column = GetDataFieldName(mGrid.Columns[colindex]);

            if (g == null)
                list = mGeneralSummaries;
            else
                list = g.Summaries;
        ...
        public int GetVisibleColumnIndex(string columnName)
        {
            ...
            for (int i = 0; i < mGrid.Columns.Count; i++)
            {
                if (GetDataFieldName(mGrid.Columns[i]).ToLower() == columnName)
                {
                    return visibles;
                }
        ...
        {
            HideColumnsWithoutGroupSummary();
        }
        ...
        {
            mGrid.Sort(GetSequentialGroupColumns(), groupSortDir);
        }
