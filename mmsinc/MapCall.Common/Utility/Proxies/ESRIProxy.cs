...
namespace MapCall.Common.Utility.Proxies
{
    /// <summary>
    ///
    /// DotNet proxy client.
    ...
    /// 
    /// </summary>
    public class ESRIProxy : IHttpHandler
    {
        #region Constants
        ...
        private static System.Net.IWebProxy
            SYSTEM_PROXY = System.Net.HttpWebRequest.DefaultWebProxy; // Use the default system proxy
        ...
        {
            ProcessRequest(context, RETRY_COUNT);
        }

        public void ProcessRequest(HttpContext context, int retryCount)
        {
            ...
            {
                ESRIProxyConfig proxyConfig = ESRIProxyConfig.GetCurrentConfig();
            ...
            if (uri.StartsWith("http%3a%2f%2f", StringComparison.InvariantCultureIgnoreCase) ||
                uri.StartsWith("https%3a%2f%2f", StringComparison.InvariantCultureIgnoreCase))
            ...
            try
            {
                ...
                if (serverUrl == null)
                {
                    ...
                    string errorMsg =
                        "The request URL does not match with the ServerUrl in proxy.config! Please check the proxy.config!";
                    log(TraceLevel.Error, errorMsg);
                    sendErrorResponse(context.Response, null, errorMsg, System.Net.HttpStatusCode.BadRequest);
                    return;
                }
            ...
            String[] allowedReferersArray = ESRIProxyConfig.GetAllowedReferersArray();
            if (allowedReferersArray != null && allowedReferersArray.Length > 0 &&
                context.Request.Headers["referer"] != null)
            {
                PROXY_REFERER = context.Request.Headers["referer"];
                string requestReferer = context.Request.Headers["referer"];
                try
                {
                    String checkValidUri = new UriBuilder(requestReferer.StartsWith("//")
                        ? requestReferer.Substring(requestReferer.IndexOf("//") + 2)
                        : requestReferer).Host;
                }
                catch (Exception e)
                {
                    log(TraceLevel.Warning,
                        "Proxy is being used from an invalid referer: " + context.Request.Headers["referer"]);
                    sendErrorResponse(context.Response, "Error verifying referer. ",
                        "403 - Forbidden: Access is denied.", System.Net.HttpStatusCode.Forbidden);
                    return;
                }
                ...
                {
                    log(TraceLevel.Warning,
                        "Proxy is being used from an unknown referer: " + context.Request.Headers["referer"]);
                    sendErrorResponse(context.Response, "Unsupported referer. ", "403 - Forbidden: Access is denied.",
                        System.Net.HttpStatusCode.Forbidden);
                }
            }

            //Check to see if allowed referer list is specified and reject if referer is null
            if (context.Request.Headers["referer"] == null && allowedReferersArray != null &&
                !allowedReferersArray[0].Equals("*"))
            {
            ...
            if (serverUrl.RateLimit > -1)
            {
                lock (_rateMapLock)
                {
                    ConcurrentDictionary<string, RateMeter> ratemap =
                        (ConcurrentDictionary<string, RateMeter>)context.Application["rateMap"];
                    if (ratemap == null)
                    {
                    ...
                    {
                        log(TraceLevel.Warning,
                            " Pair " + key + " is throttled to " + serverUrl.RateLimit + " requests per " +
                            serverUrl.RateLimitPeriod + " minute(s). Come back later.");
                        sendErrorResponse(context.Response,
                            "This is a metered resource, number of requests have exceeded the rate limit interval.",
                            "Unable to proxy request for requested resource", (System.Net.HttpStatusCode)429);
                        return;
                    }
            ...
            {
                credentials =
                    new System.Net.NetworkCredential(serverUrl.Username, serverUrl.Password, serverUrl.Domain);
            }
            else
            {
                //if token comes with client request, it takes precedence over token or credentials stored in configuration
                hasClientToken = requestUri.Contains("?token=") || requestUri.Contains("&token=") ||
            ...
            try
            {
                serverResponse = forwardToServer(context.Request, addTokenToUri(requestUri, token, tokenParamName),
                    postBody, credentials);
            }
            catch (System.Net.WebException webExc)
            {
                // IF WE RECEIVE A 498, GET A NEW TOKEN AND TRY AGAIN
                if (retryCount > 0 && ((HttpWebResponse)webExc.Response).StatusCode == (HttpStatusCode)498)
                {
                    context.Application.Lock();
                    context.Application["token_for_" + serverUrl.Url] =
                        getNewTokenIfCredentialsAreSpecified(serverUrl, requestUri);
                    context.Application.UnLock();
                    ProcessRequest(context, retryCount - 1);
                    return;
                }
        ...
        private System.Net.WebResponse forwardToServer(HttpRequest req, string uri, byte[] postBody,
            System.Net.NetworkCredential credentials = null)
        {
        ...
        private bool fetchAndPassBackToClient(System.Net.WebResponse serverResponse, HttpResponse clientResponse,
            bool ignoreAuthenticationErrors)
        {
        ...
        private System.Net.HttpWebRequest createHTTPRequest(string uri, string method, string contentType,
            System.Net.NetworkCredential credentials = null)
        {
        ...
        private string getNewTokenIfCredentialsAreSpecified(ServerUrl su, string reqUrl)
        {
            ...
            if (isUserLogin || isAppLogin)
            {
                ...
                if (isAppLogin)
                {
                    ...
                    string uri = su.OAuth2Endpoint + "token?client_id=" + su.ClientId + "&client_secret=" +
                                 su.ClientSecret + "&grant_type=client_credentials&f=json";
                ...
                else
                {
                    ...
                    else if (reqUrl.ToLower().Contains("/sharing/"))
                    {
                        ...
                        throw new ApplicationException(
                            "Unable to determine the correct URL to request a token to access private resources.");
                    ...
                    if (infoUrl != "")
                    {
                        ...
                        {
                            log(TraceLevel.Info,
                                " Service is secured by " + tokenServiceUri + ": getting new token...");
                            string uri = tokenServiceUri + "?f=json&request=getToken&referer=" + PROXY_REFERER +
                                         "&expiration=60&username=" + su.Username + "&password=" + su.Password;
                            string tokenResponse = webResponseToString(doHTTPRequest(uri, "POST"));
                            token = extractToken(tokenResponse, "token");
                        }
                    }
        ...
        private bool checkReferer(String[] allowedReferers, String referer)
        {
            if (allowedReferers != null && allowedReferers.Length > 0)
            {
                ...
                foreach (String allowedReferer in allowedReferers)
                {
                    ...
                    String allowedRefererPath =
                        allowedRefererCannonical.Substring(allowedRefererProtocol.Length + 3 +
                                                           allowedRefererDomain.Length);
        ...
        private string exchangePortalTokenForServerToken(string portalToken, ServerUrl su)
        {
            ...
            string uri =
                su.OAuth2Endpoint.Substring(0,
                    su.OAuth2Endpoint.IndexOf("/oauth2/", StringComparison.OrdinalIgnoreCase)) +
        ...
        private static void sendErrorResponse(HttpResponse response, String errorDetails, String errorMessage,
            System.Net.HttpStatusCode errorCode)
        {
            String message = string.Format("{{\"error\": {{\"code\": {0},\"message\":\"{1}\"", (int)errorCode,
                errorMessage);
        ...
        private static ESRIProxyConfig getConfig()
        {
            ESRIProxyConfig config = ESRIProxyConfig.GetCurrentConfig();
            if (config != null)
                return config;
            else
                    ...
                    string
                        configDirectory =
                            HttpContext.Current.Server
                                       .MapPath(
                                            "ESRIproxy.config"); //Cannot use System.Web.Hosting.HostingEnvironment.ApplicationPhysicalPath b/ config may be in a child directory
                    string path = configDirectory.Replace("ESRIproxy.config", "");
                    log = path + log;
                }
    ...
    [XmlRoot("ProxyConfig")]
    public class ESRIProxyConfig
    {
        private static object _lockobject = new object();

        public static ESRIProxyConfig LoadProxyConfig(string fileName)
        {
            ESRIProxyConfig config = null;
            lock (_lockobject)
            {
                if (System.IO.File.Exists(fileName))
                {
                    XmlSerializer reader = new XmlSerializer(typeof(ESRIProxyConfig));
                    using (System.IO.StreamReader file = new System.IO.StreamReader(fileName))
                    {
                        try
                        {
                            config = (ESRIProxyConfig)reader.Deserialize(file);
                            SetWebConfigProxyCredentials(config);
                        }
                        catch (Exception ex)
                        {
                            throw ex;
        ...
        // Attempts to find credentials in web.config for ULRs that match and sets them
        private static void SetWebConfigProxyCredentials(ESRIProxyConfig config)
        {
            // stored in app.config as "url,un,p;url,un,p;"
            var appConfigEsriUrls = ConfigurationManager.AppSettings["esriUrls"].Split(';'); 
            foreach (var configUrlString in appConfigEsriUrls)
            {
                var esriUrl = configUrlString.Split(',');
                var matchedUrl = config.ServerUrls.SingleOrDefault(x => x.Url == esriUrl[0]);
                if (matchedUrl != null)
                {
                    matchedUrl.Username = esriUrl[1];
                    matchedUrl.Password = esriUrl[2];
                }
            }
        }

        public static ESRIProxyConfig GetCurrentConfig()
        {
            ESRIProxyConfig config = HttpRuntime.Cache["proxyConfig"] as ESRIProxyConfig;
            if (config == null)
            {
                string fileName = HttpContext.Current.Server.MapPath("ESRIproxy.config");
        ...
        public string AllowedReferers
        {
            ...
            set { allowedReferers = Regex.Replace(value, @"\s", ""); }
        }
