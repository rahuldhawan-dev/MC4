    ...
    public class IgnoreIfAttribute : Attribute
    {
        ...
        internal bool ShouldIgnore(ITestMethod testMethod)
        {
            try
            {
                ...
                var searchFlags = BindingFlags.Public | BindingFlags.NonPublic |
                                  BindingFlags.FlattenHierarchy | BindingFlags.Static;
                var method =
                    testMethod.MethodInfo.DeclaringType.GetMethod(IgnoreCriteriaMethodName, searchFlags);
                return (bool)method.Invoke(null, new object[] { testMethod });
            }
            catch (TargetInvocationException e) when (e.InnerException is AssertFailedException)
            {
