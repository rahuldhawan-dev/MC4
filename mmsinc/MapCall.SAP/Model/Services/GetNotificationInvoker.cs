...
{
    public class GetNotificationInvoker : SAPServiceInvoker
        <GetNotificationAggregate, GetNotification_OB_SYNClient, GetNotification_OB_SYN>
    {
        ...
        public GetNotificationInvoker(ISAPHttpClient sapHttpClient) : base(sapHttpClient)
        {
        ...
        protected override GetNotificationAggregate InvokeInternal(GetNotificationAggregate sapEntity, GetNotification_OB_SYN client)
        {
            SAPNotificationCollection sapNotificationCollection = new SAPNotificationCollection();
            SAPNotification sapNotification;
            try
            {
                var notificationRecord = GetNotificationRequest(sapEntity.SearchSapNotifications);
        ...
        /// <summary>
        /// Assign search notification values to web service request
        /// </summary>
        /// <param name="searchSapNotification"></param>
        /// <returns></returns>
        private GetNotification GetNotificationRequest(SearchSapNotification searchSapNotification)
        {
            ...
            notifications[0] = new GetNotificationNotifications {
                PlanningPlant = searchSapNotification.PlanningPlant,
                DateCreatedFrom = searchSapNotification.DateCreatedFrom,
                DateCreatedTo = searchSapNotification.DateCreatedTo,
                NotificationType = searchSapNotification.NotificationType,
                Code = searchSapNotification.Code,
                CodeGroup = searchSapNotification.CodeGroup,
                Priority = searchSapNotification.Priority
            };
