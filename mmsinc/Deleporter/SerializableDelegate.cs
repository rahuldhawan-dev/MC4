    ...
    internal class SerializableDelegate<TDelegate> : ISerializable where TDelegate : class
    {
        ...
        internal SerializableDelegate(SerializationInfo info, StreamingContext context)
        {
            ...
            else
            {
                ...
                var anonymousClassWrapper =
                    (AnonymousClassWrapper)info.GetValue("class", typeof(AnonymousClassWrapper));
                Delegate = (TDelegate)(object)System.Delegate.CreateDelegate(delegateType,
                    anonymousClassWrapper.TargetInstance, methodInfo);
            }
        }
        ...
        void ISerializable.GetObjectData(SerializationInfo info, StreamingContext context)
        {
            ...
            if ((untypedDelegate.Target == null || untypedDelegate.Method.DeclaringType
                                                                  .GetCustomAttributes(typeof(SerializableAttribute),
                                                                       false).Length > 0) && Delegate != null)
            {
            ...
            else
            {
                ...
                info.AddValue("class",
                    new AnonymousClassWrapper(untypedDelegate.Method.DeclaringType, untypedDelegate.Target));
            }
        }
        ...
        private class AnonymousClassWrapper : ISerializable
        {
            ...
            internal AnonymousClassWrapper(SerializationInfo info, StreamingContext context)
            {
                ...
                foreach (FieldInfo field in classType.GetFields())
                {
                    if (typeof(Delegate).IsAssignableFrom(field.FieldType))
                        ...
                        field.SetValue(TargetInstance,
                            ((SerializableDelegate<TDelegate>)info.GetValue(field.Name,
                                typeof(SerializableDelegate<TDelegate>))).Delegate);
                    else if (!field.FieldType.IsSerializable)
                        //If the field is an anonymous class
                        field.SetValue(TargetInstance,
                            ((AnonymousClassWrapper)info.GetValue(field.Name, typeof(AnonymousClassWrapper)))
                           .TargetInstance);
                    else
                        //otherwise
            ...
            void ISerializable.GetObjectData(SerializationInfo info, StreamingContext context)
            {
                ...
                foreach (FieldInfo field in targetType.GetFields())
                {
                    ...
                    if (typeof(Delegate).IsAssignableFrom(field.FieldType))
                        info.AddValue(field.Name,
                            new SerializableDelegate<TDelegate>((TDelegate)field.GetValue(TargetInstance)));
                    else if (!field.FieldType.IsSerializable)
                        info.AddValue(field.Name,
                            new AnonymousClassWrapper(field.FieldType, field.GetValue(TargetInstance)));
                    else
