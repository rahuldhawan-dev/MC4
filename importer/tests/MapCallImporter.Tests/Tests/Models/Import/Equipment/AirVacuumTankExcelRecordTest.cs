...
{
    [TestClass]
    public class AirVacuumTankExcelRecordTest : EquipmentExcelRecordTestBase<AirVacuumTankExcelRecord>
    {
        #region Private Methods

        protected override AirVacuumTankExcelRecord CreateTarget()
        {
        ...
        protected override string ExpectedIdentifier => "NJ7-10-AVTG-1";

        #endregion

        #region Mapping

        protected override void TestCharacteristicMappings(EquipmentCharacteristicMappingTester<AirVacuumTankExcelRecord> test)
        {
            test.DropDown(x => x.Application, "APPLICATION_TNK-PVAC");
            test.DropDown(x => x.IndoorOutdoor, "LOCATION");
            test.String(x => x.OwnedBy, "OWNED_BY");
            test.DropDown(x => x.SpecialMtnNote, "SPECIAL_MAINT_NOTES_DIST");
            test.DropDown(x => x.TankAirVacuum, "TNK-PVAC_TYP");
