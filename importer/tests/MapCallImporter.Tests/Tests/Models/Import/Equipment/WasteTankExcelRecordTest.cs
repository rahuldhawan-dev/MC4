...
{
    [TestClass]
    public class WasteTankExcelRecordTest : EquipmentExcelRecordTestBase<WasteTankExcelRecord>
    {
        #region Private Methods

        protected override WasteTankExcelRecord CreateTarget()
        {
        ...
        protected override string ExpectedIdentifier => "NJ7-10-WTKG-1";
        ...
        protected override void TestCharacteristicMappings(
            EquipmentCharacteristicMappingTester<WasteTankExcelRecord> test)
        {
            test.DropDown(x => x.Application, "APPLICATION_TNK-WSTE");
            test.DropDown(x => x.IndoorOutdoor, "LOCATION");
            test.String(x => x.OwnedBy, "OWNED_BY");
            test.DropDown(x => x.SpecialMtnNote, "SPECIAL_MAINT_NOTES_DIST");
            test.DropDown(x => x.TankWaste, "TNK-WSTE_TYP");
